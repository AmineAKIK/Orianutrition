...
export function BlogPage() {
  ...
  const sorted = useMemo(
    () => [...articles].sort((a, b) => b.isoDate.localeCompare(a.isoDate)),
    [],
  );
  const [featured, ...rest] = sorted;
  const categories = useMemo(
    () => ["Toutes", ...Array.from(new Set(articles.map((a) => a.category)))],
    [],
  );
  const filtered =
    active === "Toutes" ? rest : rest.filter((a) => a.category === active);
    ...
    <section className="section-pad bg-paper">
      <Container>
        <SectionHeading
          level={1}
          eyebrow="Conseils"
          title="Des repères pour ton bien-être au quotidien"
          body="Horaires atypiques, sommeil, repas et organisation : des articles pour comprendre et avancer sans raccourci."
        />
        <div
          ...
          >
            <div className="aspect-[4/3] overflow-hidden border border-sage bg-cream sm:aspect-[3/2]">
              <img
                ...
                style={{ objectPosition: articleImagePositions[featured.slug] }}
                decoding="async"
              />
            </div>
            <div>
              <p className="eyebrow mb-3 sm:mb-4">{featured.category}</p>
              ...
              <span className="mt-6 inline-flex items-center gap-1.5 text-sm font-medium text-forest">
                Lire l'article <ArrowUpRight size={15} aria-hidden="true" />
              </span>
            </div>
          </Link>
        )}
