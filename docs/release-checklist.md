...
- Public routes are deliberately indexable for the v1 release.
...
The route matrix checks the main landmark, one `h1`, console/page errors and WCAG 2 A/AA axe rules, blocking critical and serious violations. Interaction tests also cover skip-link routing/focus, mobile navigation focus restoration and the orientation quiz flow.
...
2. Publish a GitHub Release named `v1.0.0` summarizing routing/accessibility hardening, the meaningful orientation quiz, metadata/PWA stabilization, systemic quality gates, performance optimization, portfolio framing and the final public-release matrix.
