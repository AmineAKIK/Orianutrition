...
import { calculateSleepGap, durationToMinutes, formatDuration } from '../src/lib/sleep.ts'
...
test('reports an observed sleep gap without pseudo-precision', () => {
  assert.deepEqual(calculateSleepGap({ hours: 7, minutes: 30 }, { hours: 6, minutes: 15 }), {
    targetMinutes: 450,
    observedMinutes: 375,
...
test('distinguishes aligned and above-target durations', () => {
  assert.equal(calculateSleepGap({ hours: 7, minutes: 0 }, { hours: 7, minutes: 0 }).status, 'aligned')
  assert.equal(calculateSleepGap({ hours: 7, minutes: 0 }, { hours: 7, minutes: 45 }).status, 'above')
})
