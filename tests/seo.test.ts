...
test("accepts content-derived metadata for detail routes", () => {
  const metadata = resolveRouteMetadata(
    ...
    {
      title: "Bowl quinoa, courge & pois chiches — Oria Nutrition",
      description: "Une recette pensée pour un poste.",
    },
    false,
  );
  assert.equal(
    metadata.title,
    "Bowl quinoa, courge & pois chiches — Oria Nutrition",
  );
...
test("keeps the client space and unknown routes out of indexing and crawling", () => {
  assert.equal(
    resolveRouteMetadata("/espace-client", undefined, true).robots,
    "noindex,nofollow",
  );
  assert.equal(
    resolveRouteMetadata("/inconnue", undefined, true).title,
    "Page introuvable — Oria Nutrition",
  );
  assert.equal(
    resolveRouteMetadata("/inconnue", undefined, true).robots,
    "noindex,nofollow",
  );
});
