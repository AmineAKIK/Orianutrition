...
for (const route of routes) {
  test(`${route} loads with core accessibility invariants`, async ({ page }) => {
...
test('skip link preserves the hash route and focuses main', async ({ page }) => {
...
test('orientation answers produce a tailored recommendation and can reset', async ({ page }) => {
  await page.goto('#/accompagnements')
  await page.getByRole('button', { name: 'Trouver mon accompagnement' }).click()
  ...
  await expect(page.getByRole('heading', { name: 'Commencer par un bilan structuré' })).toBeVisible()
